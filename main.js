...
// --- Get DOM Elements (This script runs after the body, so elements are available) ---
...
        remove(ref(database, `targets/${selectedMonth}`));
        incomeTargetInput.value = '';
        expenseTargetInput.value = '';
...
        item.className = `flex justify-between items-center p-2 rounded-md mb-2 ${
            transaction.type === 'income' ? 'bg-green-100 dark:bg-green-900' : 
            transaction.type === 'expense' ? 'bg-red-100 dark:bg-red-900' : 'bg-blue-100 dark:bg-blue-900'
        }`;
        item.innerHTML = `
            <div>
    ...
                <button class="text-gray-400 hover:text-red-500 text-lg" onclick="removeTransaction('${transaction.id}')">&times;</button>
            </div>
        `;
...
        incomeProgressText.innerText = 'No target set';
        expenseProgressText.innerText = 'No target set';
        savingProgressText.innerText = 'No target set';
        return;
    }
...
    const transactionMonths = allTransactions.map(t => t.date ? t.date.substring(0, 7) : null).filter(Boolean);
...
    } else if (allMonthStrings.includes(currentMonthStr)) {
        monthFilter.value = currentMonthStr;
    } else if (allMonthStrings.length > 0) {
        monthFilter.value = allMonthStrings[0];
    } else {
...
        monthlyViewBtn.classList.add('bg-blue-500', 'text-white');
        weeklyViewBtn.classList.remove('bg-blue-500', 'text-white');
        updateUI(); 
    }
});
...
        weeklyViewBtn.classList.add('bg-blue-500', 'text-white');
        monthlyViewBtn.classList.remove('bg-blue-500', 'text-white');
        updateUI();
    }
});
...
// Initial data load is now handled by onValue listeners.
