...
function generateMonthlyChartData(monthlyTransactions, selectedMonth) {
    ...
    return {
        ...
        datasets: [
            ...
            { label: 'Expense & Savings', data: expenseData, backgroundColor: 'rgba(231, 76, 60, 0.7)' }
        ]
    };
}
...
function updateChart(monthlyTransactions, selectedMonth) {
    ...
    if (selectedMonth === 'all' || monthlyTransactions.length === 0) {
        ...
        ctx.fillText("No data to display for this period.", financeChartCanvas.width / 2, financeChartCanvas.height / 2);
        return;
    }
    ...
    if (currentChartView === 'weekly') {
        chartData = generateWeeklyChartData(monthlyTransactions);
        xAxisTitle = `Weeks in ${monthName}`;
    } else {
        chartData = generateMonthlyChartData(monthlyTransactions, selectedMonth);
        xAxisTitle = `Days in ${monthName}`;
    }
...
function updateTransactionLists(transactionsToDisplay) {
    ...
    sortedTransactions.forEach(transaction => {
        ...
        const bgColorClass = transaction.type === 'income' ? 'bg-green-100 dark:bg-green-900/50' : 
                             transaction.type === 'expense' ? 'bg-red-100 dark:bg-red-900/50' : 
                             'bg-blue-100 dark:bg-blue-900/50';

        item.className = `flex justify-between items-center p-2 rounded-md mb-2 ${bgColorClass}`;
        
        item.innerHTML = `
            <div>
                <span>${transaction.description}</span>
                <span class="text-xs text-gray-500 dark:text-gray-400 block">${transaction.date}</span>
            </div>
            <div class="flex items-center">
                <span class="font-semibold mr-4">Rs ${transaction.amount.toFixed(2)}</span>
                <button class="delete-btn text-gray-400 hover:text-red-500 text-lg" data-id="${transaction.id}">&times;</button>
            </div>
        `;
...
function updateMonthlyAnalysis(monthlyTransactions) {
    if (monthFilter.value === 'all') {
        monthlySummary.innerHTML = '<p class="text-center text-gray-500">Select a specific month to see its analysis.</p>';
        return;
    }
    ...
    monthlySummary.innerHTML = `
        <div class="space-y-2">
            <p class="flex justify-between"><span>Income:</span> <span class="font-semibold text-green-500">Rs ${income.toFixed(2)}</span></p>
            <p class="flex justify-between"><span>Expenses:</span> <span class="font-semibold text-red-500">Rs ${expense.toFixed(2)}</span></p>
            <p class="flex justify-between"><span>Savings:</span> <span class="font-semibold text-blue-500">Rs ${savings.toFixed(2)}</span></p>
            <hr class="border-gray-300 dark:border-gray-600 my-2">
            <p class="flex justify-between text-lg"><strong><span>Net for month:</span> <span>Rs ${(income - expense - savings).toFixed(2)}</span></strong></p>
        </div>
    `;
}
...
function updateTargetProgress(monthlyTransactions, selectedMonth) {
    ...
    const updateBar = (bar, text, current, target) => {
        if (!target || target <= 0) {
            bar.style.width = '0%';
            text.innerText = 'No target set';
        } else {
            const percent = Math.min((current / target) * 100, 100);
            bar.style.width = `${percent}%`;
            text.innerText = `Rs ${current.toFixed(2)} / Rs ${target.toFixed(2)}`;
        }
    };
    ...
    updateBar(incomeProgressBar, incomeProgressText, currentIncome, targets?.income);
    updateBar(expenseProgressBar, expenseProgressText, currentExpense, targets?.expense);
    updateBar(savingProgressBar, savingProgressText, currentSaving, targets?.saving);
}
...
function populateMonthFilter() {
    ...
    const months = new Set([
        ...
        ...Object.keys(allTargets),
        ...allTransactions.map(t => t.date ? t.date.substring(0, 7) : null).filter(Boolean)
    ]);
    ...
    if (currentSelected && months.has(currentSelected)) {
        monthFilter.value = currentSelected;
    } else if (months.has(new Date().toISOString().substring(0, 7))) {
...
function setupEventListeners() {
    addIncomeBtn.addEventListener('click', () => processTransaction('income'));
    addExpenseBtn.addEventListener('click', () => processTransaction('expense'));
    addSavingBtn.addEventListener('click', () => processTransaction('saving'));
    setTargetBtn.addEventListener('click', setTargets);
    clearTargetsBtn.addEventListener('click', clearMonthlyTargets);
    monthFilter.addEventListener('change', updateUI);
    toggleTargetsBtn.addEventListener('click', () => targetForm.classList.toggle('hidden'));
    ...
    [incomeList, expenseList, savingList].forEach(list => list.addEventListener('click', handleListClick));

    const setChartView = (view) => {
        if (currentChartView !== view) {
            currentChartView = view;
            monthlyViewBtn.classList.toggle('bg-blue-500', view === 'monthly');
            monthlyViewBtn.classList.toggle('text-white', view === 'monthly');
            weeklyViewBtn.classList.toggle('bg-blue-500', view === 'weekly');
            weeklyViewBtn.classList.toggle('text-white', view === 'weekly');
            updateUI();
        }
    };
    monthlyViewBtn.addEventListener('click', () => setChartView('monthly'));
    weeklyViewBtn.addEventListener('click', () => setChartView('weekly'));

    // ADDED: Theme Toggle Logic
    const updateThemeIcon = () => {
        const isDark = document.documentElement.classList.contains('dark');
        themeToggleBtn.innerHTML = isDark 
            ? `<i class="fas fa-sun w-5 text-center"></i> Light Mode`
            : `<i class="fas fa-moon w-5 text-center"></i> Dark Mode`;
    };
...
function initializeAppDashboard() {
    ...
    setupEventListeners();
    // Initial data load and UI update is handled by the `onValue` listeners,
    // but calling this ensures the UI renders correctly on first load.
    updateUI();
}

// FIX: Calling the renamed function
